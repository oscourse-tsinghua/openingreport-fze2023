...
// 调研A，利用B，实现C，达成D效果
本研究工作对区块链技术在以操作系统为代表的计算机行业内应用情况进行调研。在基于Rust的开源操作系统内核项目rCore的基础上，以系统调用的引入开源区块链Substrate的基于Rust的节点实现Substrate Node Template的部分功能，令rCore具备原生的区块链交互开发能力，减轻上层应用开发者开发分布式app的心智负担。主要工作分为三阶段展开：

第一阶段针对Substrate Node Template项目展开。作为官方给出的精简版节点实现，Substrate Node Template具备管理账号、转储余额等基本机能，同时也支持通过引入“pallet”的形式提供更多诸如智能合约的功能。该阶段的主要目标为：对项目代码进行分析以确定其依赖项，确认rCore内核提供的服务是否能满足这些依赖项的要求。若不满足，则需确定需要进一步实现的系统调用。

第二阶段针对rCore进行完善工作。（rCore的简要介绍）。按照第一阶段的依赖分析，逐一将依赖引入rCore中。对于rCore缺少的系统调用，参考已有的系统调用和Linux等开源项目的实现进行补充。预期该阶段完成后，Substrate Node Template应当可以以用户程序的形态在rCore中运行。

第三阶段，将Substrate Node Template从用户态迁移向内核态，并将区块链开发的关键操作以新系统调用的形式向用户程序开放。具体而言，这将包含账号管理、余额管理和智能合约三大类功能，每一类功能下设数个系统调用，供上层应用开发者使用。该阶段完成后，可编写简单的用户态测试程序，验证区块链操作系统的功能完备性和正确性。
