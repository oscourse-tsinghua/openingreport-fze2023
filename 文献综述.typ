...
虽然死代码并不会被执行，但已有研究证明它们对软件开发与维护仍具有负面影响@unused_code_matters ，其中文章@MultiStudy 将死代码的负面影响归结为了以下几点：
...
常见的代码复用手段主要包含软件模块化设计和软件移植。前者将大型软件拆分成小的可复用的模块，供其他开发者选用并成为他们所开发的软件的一部分；后者将大型软件在不同的操作系统甚至不同架构的硬件平台上进行移植，以拓宽同样功能代码的泛用性@Ray2012ACS
。
...
上述两项工作的共同结论之一是，代码行数的增加会令移植工作更具挑战性。这从侧面证明了死代码移除对降低移植难度的重要性：通过移除不必要的代码，减少代码行数，从而降低移植工作难度，提升移植效率。

== 死代码探测与移除技术

死代码探测技术，即在源代码中查找符合上一章定义的死代码的技术方案。除降低软件复杂性外，死代码探测技术亦有许多用途，例如文章 @through_lens_of_dead_code_elimination
提出以死代码探测评估软件质量高低的方法。目前，针对不同编程语言的死代码移除技术不断涌现，其中较具代表性的有JavaScript的Tree Shaking技术方案，PHP的Web系统的动态标记技术方案和Java的DUM技术方案。
...
。针对一些C语言函数调用图生成工具存在的需要基础知识、产生过量冗余信息、和编译环境耦合度过高的问题，文献 @cg_rtl 提出一种基于寄存器传送语言（Register Transfer
...
其次，针对提出的 Rupta 改造方案，将编制计划进行方案的具体实现与优化。重点关注改造过程中的兼容性问题，确保 Rupta 在稳定运行的同时，具备输出便于分析的期望信息的能力。同时，积极探索 Rupta
