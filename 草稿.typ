...
本课题提出开发一款针对rust程序设计语言的优化工具，其能够以函数调用作为最小单元对指定的rust项目（即rust crate）进行分析，识别其中及其依赖项中的冗余代码，为开发者给出代码裁剪建议。为证明该工具确实能有效辅助进行跨架构软件移植，本课题将以rust实现的Substrate区块链为例，使用该工具进行分析，依据其给出的裁剪建议进行裁剪，统计裁剪前后代码的体积变化，并与未裁剪的原始代码进行比较，证明该工具的有效性。随后，尝试将裁剪完成的Substrate移植入rCore教学操作系统中，在移植过程中实现必要的系统调用，为rCore实现内核层级的原生区块链功能支持。
