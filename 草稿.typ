...
虽然有诸多依赖项需要修改，但在测试过程中发现的一些现象亦值得注意。多数依赖项在编译报错时的错误信息具有极强的共性，例如找不到`Ok`、`Result`等常见的Rust结构体等。这些错误信息表明，依赖项在编译时使用了rust-std，而rCore操作系统没有提供这一库。只需将其替换为rust核心库（core）中具有相同功能的同名结构体，问题便可解决。
...
- 2024年6月：开题，方案设计。
- 2024年7月～2024年8月：死代码探测技术方案实现、开展移植工作。
- 2024年9月～2024年11月：Substrate Node Template的rCore操作系统兼容性改写。
