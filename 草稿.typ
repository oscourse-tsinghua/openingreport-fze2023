...
MIRAI工作在rust编程语言的MIR中间表示上，其最小分析粒度为函数。由rust中间表示的特性可知，自THIR开始，rustc就抛弃了诸如`struct`、`impl`块等内容，而将所有的方法均展开成为了普通的函数定义。在THIR及之后的中间表示中，函数定义始终以`body`的形式存在，且每个函数均以被称为`DefId`的编号唯一标识。这种行为非常符合选题构造函数调用图的需求。

正如前文所言，rustc为MIR提供了一组不稳定的API供开发者调用，其中提供的`rustc_driver::Callbacks`特征（`trait`）允许开发者介入rustc编译rust项目的不同阶段，获取编译信息并执行自定义的回调函数进行分析等操作。MIRAI实现了该特征中的`after_analysis`回调方法，该回调方法在rustc编译获得MIR之后，继续降层为LLVM IR之前调用，可获得编译器在将源代码编译到MIR中间表示中收集的所有信息。通过该方法，MIRAI进行了自顶向下，由最大范围的rust包（crate）级分析到最小范围的函数调用级分析的过程，其调用链大致如@img1 所示。
...
整体内容和MIRAI现阶段的分析结果输出相比结构和内容均有不同，主要增加了crate的完整名称及位置信息，方便开发者区分同一crate的不同版本，更有针对性地进行死代码探测及移除。
...
- 2024年6月：开题，方案设计。
- 2024年7月～2024年11月：死代码探测技术方案实现、开展移植工作。
