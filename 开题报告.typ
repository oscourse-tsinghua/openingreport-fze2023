...
本选题提出一种基于函数调用图进行死代码探测的技术方案，帮助开发者更高效地移除死代码，在简化软件业务逻辑理解的同时，降低代码库体积，提升软件运行效率；为验证这一方案的可行性，本选题还将以实际rust软件项目为对象进行死代码移除，并移植到真实操作系统中，使之成为顺应Web
3.0时代潮流的区块链操作系统。
...
。而在程序设计语言领域，死代码是指其结果从未被使用的计算（例如，在代码中引用的变量，但在运行时实际上不使用）@CodeCompaction 。虽定义不同，其本质并无二样，即死代码是在程序运行过程中永不可能被执行的部分。

虽然死代码并不会被执行，但它们对软件开发与维护仍具有负面影响，主要可以归结为以下几点@MultiStudy：

- 令代码更难理解: 开发者更难理解代码的结构和用意@BetterUnderstanding @MetricsUnderstanding
  。对于经验尚不丰富的新开发人员，他们可能会误以为死代码是有用的，从而轻则花费不必要的时间弄懂死代码、重则任其累积令代码库的质量越来越差。
...
Tree
Shaking技术方案依赖ES2015标准引入的`import`、`export`模块语法进行死代码移除。它通过分析依赖关系，确定未使用的模块并在最终生成的代码中将它们剔除。然而其简单的技术原理与思想也带来了局限性：它只能以模块为单位，而不能以模块中具体的功能为单位进行移除，导致生成的源代码体积仍然过大；另外，该过程对开发者透明，开发者无从知晓哪些模块被删除，因此无法辅助开发者主动提高代码质量。
...
编译型程序设计语言的编译工具链在将源代码编译为目标平台的二进制可执行文件时，往往会选择先将源代码编译为某种形式的中间表示（IR），再将中间表示编译为目标平台的机器代码。文献 @cg_rtl 提出一种基于寄存器传送语言（Register
Transfer
...
Substrate由以太坊项目的联合创始人Gavin
Wood率领Parity团队开发，是一个基于rust的开源区块链框架。它对于多链结构提供了较好的支持，能够克服传统单链区块链在一条链上保有大量用户、存储大量信息，导致运行效率降低的问题，且针对多链间通信提出了一套解决方案。不仅如此，高度可定制的灵活模块化设计模式，使得Substrate能够以功能模块（称之为pallet）为单位进行组件增删，令开发者得以将Substrate打造成最契合他们需求的区块链。
...
SNT的代码库如@img1 所示，其本质是一个Rust工作空间（workspace），内含三个成员：node,
runtime和pallets。这其中，pallets成员存储SNT使用的所有自定义功能模块（即上文中提及的pallet），内仅含一个模板pallet，称之为template；runtime成员主要定义了SNT在运行时的链上状态转换逻辑，为单`lib.rs`文件结构；node成员负责P2P网络通信、区块产生和确认（finalization）、处理外部RPC请求等链外事务，内含多个源代码文件，有继续细分的必要。
...
#figure(image("report.assets/SNT代码结构.drawio.png", width: 60%), caption: [Substrate Node Template代码结构])<img1>
#blank

SNT的大致依赖关系如@img2 所示。结合上文对各模块功能的描述，可给出一拓扑排序，指示令SNT自底向上摆脱具体操作系统依赖的实现顺序。

#figure(image("report.assets/SNT大致依赖关系.png", width: 60%), caption: [Substrate Node Template大致依赖关系])<img2>
...
目前，市面上尚无针对rust编程语言进行死代码探测的工具。故本选题计划以Rupta为基底，以死代码探测为目标进行修改，去除不必要的功能并完善函数调用图生成功能，以构造基于函数调用图的死代码探测技术方案。为验证该方案的可行性，取Substrate全节点客户端作为待分析项目，探测并移除其死代码，获得精简的Substrate全节点客户端。
...
Rupta实现了该特征中的`after_analysis`回调方法，该回调方法在rustc编译获得MIR之后，继续降层为LLVM
IR之前调用，可获得编译器在将源代码编译到MIR中间表示中收集的所有信息。通过该方法，Rupta进行了自顶向下，由最大范围的rust包（crate）级分析到最小范围的函数调用级分析的过程。
...
随着嵌入式开发在rust社区中的兴起，在无标准库支持的设备上进行rust开发的需求水涨船高，一个名为no-std特性的概念也应运而生。若一个crate声称自己对no-std环境兼容，则该crate能够在无标准库支持的环境下提供至少一部分功能供应用使用。在诸如crates.io和lib.rs这些rust
crate介绍平台上，支持此类特性的crate往往会为自己标注no-std的tag。在进行现有crate替换时，应尽可能采用这些支持no-std特性的crate，以降低改写工作量。
